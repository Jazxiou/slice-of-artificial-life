...
class AssociativeMemory:
    def __init__(self, f_saved):
        ...
        for count in range(len(nodes_load.keys())):
            ...
            if node:
                retention.restore_access(node, node_details)
    ...
    def save(self, out_json):
        ...
        for count in range(len(self.id_to_node.keys()), 0, -1):
            ...
            # Read only when `recency_access_persisted` is on.
            r[node_id]["last_accessed"] = node.last_accessed.strftime("%Y-%m-%d %H:%M:%S")
            r[node_id]["rehearsal_count"] = getattr(node, "rehearsal_count", 0)
    ...
    def add_chat(self, created, expiration, s, p, o, description, keywords, poignancy, embedding_pair, filling):
        # Setting up the node ID and counts.
        node_count = len(self.id_to_node.keys()) + 1
