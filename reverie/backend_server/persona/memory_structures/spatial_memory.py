...
class MemoryTree:
    ...
    def get_str_accessible_arena_game_objects(self, arena):
        ...
        # Upstream tried the name as given, then lower-cased inside a
        # simple bare `except:`. When neither was present the second lookup
        # produced a confusing traceback and gave no hint what the valid
        # names are. Fixed now.
        arenas = self.tree.get(curr_world, {}).get(curr_sector, {})
        for name in (curr_arena, curr_arena.lower()):
            if name in arenas:
                return ", ".join(list(arenas[name]))
        raise KeyError(
            f"No arena {curr_arena!r} in sector {curr_sector!r} (world {curr_world!r}). "
            f"Arenas that do exist there: {', '.join(arenas) or '(none)'}. "
            f"This means an action address was built from a place name that is not on the map."
        )
