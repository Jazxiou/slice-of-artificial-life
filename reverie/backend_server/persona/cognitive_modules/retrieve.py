...
def cos_sim(a, b):
    ...
    try:
        ...
        # Handed a None, or a list with a None in it, numpy builds an
        # object array and fails with "unsupported operand type(s) for
        # *: 'NoneType' and 'NoneType'". Says what is actually wrong.
        def describe(v, name):
            if v is None:
            ...
            try:
                ...
                return f"{name} is a {type(v).__name__}, not a vector"
            return f"{name} has {bad} None entries out of {len(v)}" if bad else f"{name} looks fine"

        raise TypeError(
            f"cos_sim could not compare the vectors: "
            f"{describe(a, 'memory embedding')}; "
            f"{describe(b, 'query embedding')}. Original error: {exc}"
        ) from exc
...
def extract_relevance(persona, nodes, focal_pt):
    ...
    relevance_out = dict()
    for count, node in enumerate(nodes):
        # The embedding table is keyed by the exact text that was
        # embedded, stored as its embedding_key. So a missing or null
        # entry can be rebuilt. Re-embedding the key reproduces the
        # vector that should have been there.
        node_embedding = persona.a_mem.embeddings.get(node.embedding_key)
        if node_embedding is None:
            print(
                f"[retrieve] no stored embedding for {node.embedding_key!r} ({node.type} {node.node_id}); recomputing."
            )
        ...
        try:
            ...
            raise TypeError(f"{exc}\n  memory  : {node.embedding_key!r}\n  query   : {focal_pt!r}") from exc

    return relevance_out


def new_retrieve(persona, focal_points, n_count=30):
    """Timed wrapper around the retrieval below."""
    started = time.time()
    try:
        return _new_retrieve(persona, focal_points, n_count)
    finally:
        llm_trace.stage("retrieval", time.time() - started)


def _new_retrieve(persona, focal_points, n_count=30):
    """
