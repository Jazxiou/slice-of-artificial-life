...
def generate_new_decomp_schedule(persona, inserted_act, inserted_act_dur, start_hour, end_hour):
    ...
    return run_gpt_prompt_new_decomp_schedule(
        persona, main_act_dur, truncated_act_dur, start_time_hour, end_time_hour, inserted_act, inserted_act_dur
    )[0]
...
def revise_identity(persona):
    ...
    plan_note = ChatGPT_single_request(plan_prompt, label="revise_identity/plan_note")
    # print (plan_note)
    ...
    thought_note = ChatGPT_single_request(thought_prompt, label="revise_identity/thought_note")
    # print (thought_note)
    ...
    new_currently = ChatGPT_single_request(currently_prompt, label="revise_identity/currently")
    # print (new_currently)
    # print (new_currently[10:])

    # The prompt above ends "Follow this format below:\nStatus:
    # <new status>", and the reply was stored whole, marker included.
    # Every prompt reading the identity stable set therefore began
    # "Currently: Status: ..." from the second simulated day onwards.
    new_currently = re.sub(r"^\s*Status\s*:\s*", "", new_currently).strip()

    # Persona re-anchoring.
    # `reanchor` returns the proposal untouched but still records the
    # day's drift
    new_currently, drifted = persona_ext.reanchor(
        persona.scratch,
        new_currently,
        get_embedding,
        lambda prompt: ChatGPT_single_request(prompt, label="revise_identity/reanchor"),
    )
    persona.scratch.drift_log = getattr(persona.scratch, "drift_log", []) + [drifted]
    # And into the trace too.
    llm_trace.drift(p_name, drifted)
    ...
    # Upstream ended here by generating a fresh broad-strokes plan for the day and assigning it to
    # `daily_plan_req` — which is not the field for it. `daily_plan_req` holds the character's *standing*
    # requirement ("Isabella Rodriguez opens Hobbs Cafe at 8am everyday, and works at the counter until
    # 8pm"), it is part of the identity stable set, and so it appears in every prompt that asks who this
    # person is. Writing a dated to-do list into it replaced each character's routine permanently: by the
    # third simulated day Isabella's standing requirement read "3. Host Valentine's Day party activities
    # from 10:00 am to 7:00 pm", and it would have followed her into every later day. Worse, the plan
    # being put there is exactly what `_long_term_planning` below was failing to produce, so the two
    # fields were in effect swapped — the ephemeral one frozen, the permanent one rewritten daily.
    #
    # The generation now happens where it belongs, through the same prompt that writes the first day's
    # plan, and the standing requirement is left as seeded.
...
def _long_term_planning(persona, new_day):
    ...
    if new_day == "First day":
        ...
        # Upstream left `persona.scratch.daily_req = persona.scratch.daily_req` here, with its own TODO
        # above it saying a new one was needed. That assignment does nothing, so the plan written on the
        # first simulated day was the plan every later day was built from: `generate_hourly_schedule` reads
        # `daily_req` under the heading "the originally intended hourly breakdown of ... today", and on the
        # third day of a measured run it was still handing Isabella "organize decorations for the
        # Valentine's Day party from 3:00 pm to 4:00 pm" for a party that had happened the day before. The
        # same call as the first day is used, so a later day's plan is produced exactly as the first one is,
        # and it now reads the identity as re-anchored a few lines above.
        persona.scratch.daily_req = generate_first_daily_plan(persona, wake_up_hour)
