    ...
    if not persona.scratch.act_path_set:
        ...
        if "<persona>" in plan:
            ...
            if plan not in maze.address_tiles:
                ...
                # here (an expression with no assignment), so it did nothing
                # and left target_tiles as None. Raises an actual error now.
                raise KeyError(
                    f"No tiles on the map for action address {plan!r}. The address was built from a place "
                    f"name that does not exist in this world."
                )
            else:
                target_tiles = maze.address_tiles[plan]
