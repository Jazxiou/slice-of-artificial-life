...
def generate_one_utterance(maze, init_persona, target_persona, retrieved, curr_chat):
    ...
    # World layer (town runs only; both lines are empty strings with the flags off, so measured
    # conditions are untouched): how this speaker feels about the listener, from their own side of
    # the relationship, and their current mood. This is what lets the scores and moods shape what is
    # actually said rather than only what is displayed.
    world_lines = world_relationships.attitude_line(
        init_persona.scratch, target_persona.scratch.name
    ) + world_emotion.feeling_line(init_persona.scratch)
    if world_lines:
        curr_context += "\n" + world_lines.rstrip()
...
def agent_chat_v2(maze, init_persona, target_persona):
    ...
    for i in range(8):
        ...
        if last_chat:
            ...
            focal_points = [f"{relationship}", f"{init_persona.scratch.name} is {init_persona.scratch.act_description}"]
