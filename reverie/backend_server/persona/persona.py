...
class Persona:
    ...
    def move(self, maze, personas, curr_tile, curr_time):
        ...
        # World layer, one step is 10 simulated seconds of the body
        # clock, needs decay and current activity refills its need.
        world_needs.tick(self.scratch, 10 / 3600)
        # Mood triggers only this layer can see (waking, a red need recovering): edge detection every
        # step, a model call only on the step an edge fires. Inert flags-off.
        world_emotion.tick(self.scratch, lambda p: ChatGPT_single_request(p, label="world/mood"))
