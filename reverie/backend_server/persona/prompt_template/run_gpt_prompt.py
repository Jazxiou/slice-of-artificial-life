...
# Words a sentence trails off with
_TRAILING_CLAUSE = re.compile(
    r"[\s,;.]*\b(which|that|this|it|its|and|takes?|taking|took|lasts?|lasting|"
    r"requires?|requiring|needs?|spends?|spending|about|approximately|around|roughly|"
    r"another|an|a|for|in|over|up|to)\b\s*$",
    re.IGNORECASE,
)


def _trim_duration_clause(text):
    """
    Strip the dangling connective left behind when a spoken duration is
    cut off a subtask.
    """
    text = text.strip()
    while True:
...
def strip_brace_answer(gpt_response):
    """
    Pull the answer out of a reply to a prompt that ends with an open
    brace.

    Several prompts stop mid-sentence on a `{`, e.g. "...should go to
    the following area: {", a trick that worked on "completion" models,
    which simply continue the text and so may reply `Hobbs Cafe}`,
    Upstream therefore reads the answer as everything before the first
    `}`.

    A "chat" model does not continue text, it answers afresh, and often
    repeats the brace it was shown (`{Maria Lopez's room}`). Upstream's
    rule then yields `{Maria Lopez's room`, opening brace still
    attached, which fails every later lookup by name.

    Drops the leading brace.
    """
    return gpt_response.strip().lstrip("{").split("}")[0]


def offered_arenas(persona, act_world, act_sector):
    """
    The list of arenas (rooms) inside a sector that this persona is
    allowed to be told about.
    """
    arenas = persona.s_mem.get_str_accessible_sector_arenas(f"{act_world}:{act_sector}")
    offered = []
    for arena in arenas.split(", "):
        if "'s room" in arena and persona.scratch.last_name not in arena:
            continue  # someone else's bedroom, which is not on offer
        offered += [arena]
    return offered


def constrain_to_options(answer, options, what):
    """
    Hold the model to the list of choices it was given.

    The location prompts say "MUST pick one of {...}", but nothing
    checked that it did. A model that names a room which does not exist
    produces an address that resolves to no tile on the map. Now, if
    the model invents a place, fall back to a real one.
    ...
    normalised = {o.strip().lower(): o for o in options}
    hit = normalised.get(answer.strip().lower())
    if hit is not None:
        return hit

    substitute = random.choice(options)
    print(
        f"[constrain] {what}: the model answered {answer!r}, which is not one of "
        f"{options}. Substituting {substitute!r}."
    )
    return substitute


# Codepoint ranges that count as an emoji.
_EMOJI_RANGES = (
    (0x200D, 0x200D),
    (0x2190, 0x21FF),
    (0x2300, 0x23FF),
...
def keep_emoji(text):
    """
    Strip everything that is not an emoji out of a pronunciation.

    The prompt asks for "an emoji" but nothing checks that it got one.
    Qwen2.5 is multilingual and regularly answers with words instead or
    with a mixture.

    Keeping only the emoji characters sometimes returns nothing at all,
    which then fails validation and is retried before falling back to
    the neutral default. Enforces what the prompt sought out to do.
    """
    return "".join(c for c in text if any(lo <= ord(c) <= hi for lo, hi in _EMOJI_RANGES))
...
def run_gpt_prompt_task_decomp(persona, task, duration, test_input=None, verbose=False):
    ...
    def __func_clean_up(gpt_response, prompt=""):
        # Upstream normalised each line by dropping its first three
        # space-separated tokens (e.g. "2)", "Isabella", "is") then
        # split on "(duration in minutes:". That assumes every single
        # line of the reply is a numbered subtask in exactly that
        # shape.
        # Reading each line with a pattern, skipping the ones that do
        # not match, no longer depends on the reply's first line being
        # special, and strips the name if it is there.
        firstname = persona.scratch.get_str_firstname()
        subtask_line = re.compile(r"^\s*(?:\d+\s*[).]\s*)?(.*?)\(duration in minutes:\s*(-?\d+)", re.IGNORECASE)
        # The examples in the prompt put the duration in brackets, and
        # the model usually copies that. Sometimes it writes the time
        # into the sentence instead. Allow that to work too.
        prose_line = re.compile(r"^\s*(?:\d+\s*[).]\s*)(.+)", re.IGNORECASE)
        prose_minutes = re.compile(r"(\d+)\s*minutes?\b", re.IGNORECASE)
        cr = []
        for line in gpt_response.split("\n"):
            match = subtask_line.match(line)
            if not match:
                # Accept a numbered line that states its length in
                # words, taking the last time mentioned and cutting the
                # clause that states it out of the task description.
                numbered = prose_line.match(line)
                spoken = prose_minutes.search(line) if numbered else None
                if not spoken:
                    continue  # blank line or the model's closing remarks
                text = numbered.group(1)[: spoken.start() - (len(line) - len(numbered.group(1)))]
                task, minutes = _trim_duration_clause(text), int(spoken.group(1))
            else:
                task = match.group(1).strip()
                minutes = int(match.group(2))
            for prefix in (f"{firstname} is ", f"{firstname} "):
        ...
        if not cr:
            # Nothing usable. Lets __func_validate see it and ask the
            # model again rather than returning an empty list.
            raise ValueError("no subtasks with a duration could be read from the reply")
        ...
        if len(curr_min_slot) > total_expected_min:
            # Was `curr_min_slot[60]` which raised IndexErrors for
            # certain tasks.
    ...
    def __func_validate(gpt_response, prompt=""):
        ...
        try:
            ...
            print(f"[task_decomp] unreadable reply, will retry: {type(exc).__name__}: {exc}")
            return False
        return True

    def get_fail_safe():
        # Was ["asleep"]. A list of one string even though every caller
        # expects [[task, minutes], ...]. Fail-safe raised "too many
        # values to unpack". Split into sub-hour pieces.
        pieces = max(2, -(-duration // 45))  # chunks of at most 45 mins
        base, extra = divmod(duration, pieces)
        return [[task, base + (1 if i < extra else 0)] for i in range(pieces)]
  ...
  """
    ...
    # If the substack already exceeds the slot, the loop breaks
    # immediately, fin_output is empty, and [-1] raises IndexError.
    # Keep th whole task instead.
    if not fin_output:
    ...
    for decomp_task, duration in task_decomp:
        if decomp_task.strip() == task.strip():
            ret += [[task, duration]]
        else:
...
def run_gpt_prompt_action_sector(action_description, persona, maze, test_input=None, verbose=False):
    ...
    def __func_clean_up(gpt_response, prompt=""):
        cleaned_response = strip_brace_answer(gpt_response)
        return cleaned_response

    def __func_validate(gpt_response, prompt=""):
        # Used to reject anything without a closing brace.
        answer = strip_brace_answer(gpt_response)
        if not answer or "," in answer:
            return False
        # A place name, not prose.
    ...
    if output not in x:
        # Added: try a case-insensitive match first, so "hobbs cafe"
        # resolves to "Hobbs Cafe" instead of sending the agent home.
        canonical = {i.strip().lower(): i for i in x}.get(output.strip().lower())
        if canonical is not None:
            output = canonical
        else:
            print(
                f"[constrain] sector: the model answered {output!r}, which is not one of {x}. "
                f"Falling back to the persona's home sector."
            )
...
def run_gpt_prompt_action_arena(
    ...
    def create_prompt_input(action_description, persona, maze, act_world, act_sector, test_input=None):
        ...
        accessible_arena_str = ", ".join(offered_arenas(persona, act_world, act_sector))
        # END MAR 11 TEMP
    ...
    def __func_validate(gpt_response, prompt=""):
        # Used to reject anything without a closing brace and proper
        # punctuation.
        answer = strip_brace_answer(gpt_response)
        if not answer or "," in answer:
            return False
    ...
    #
    # Re-enabled the above commented out code (but improved)
    output = constrain_to_options(output, offered_arenas(persona, act_world, act_sector), f"arena in {act_sector}")
...
def run_gpt_prompt_pronunciatio(action_description, persona, verbose=False):
    ...
    # ChatGPT Plugin ===========================================================
    def __chat_func_clean_up(gpt_response, prompt=""):  ############
        # Keep the emoji and drop the rest.
        cr = keep_emoji(gpt_response.strip())
        if len(cr) > 3:
            cr = cr[:3]
        return cr

    def __chat_func_validate(gpt_response, prompt=""):  ############
        # Retry when an answer has no emoji. Fail-safe emoji is used
        # when retries are exhausted.
        try:
            return len(__chat_func_clean_up(gpt_response, prompt="")) > 0
        except Exception:
            return False
    ...
    if output != False:
        return output, [output, prompt, gpt_param, prompt_input, fail_safe]
    # When ChatGPT_safe_generate_response gives up it returns False, so
    # this returned None, and the caller crashed on `[0]` with
    # "'NoneType' object is not subscriptable", an error that names
    # neither the prompt nor the reply. Use the fail-safe now. Repeated
    # below.
...
def run_gpt_prompt_event_triple(action_description, persona, verbose=False):
    ...
    def __func_clean_up(gpt_response, prompt=""):
        cr = gpt_response.strip()
        cr = [i.strip() for i in cr.lstrip("(").split(")")[0].split(",")]
        # The prompt stops mid-tuple, for example "Output: (Isabella
        # Rodriguez,", and a completion model supplies only the two
        # remaining elements and upstream requires exactly two. Issue
        # is a chat model often restates the prompt triggering the
        # fail-safe ("is", "idle"). Fixed.
        if len(cr) > 2:
            cr = cr[-2:]
        return cr
    ...
    def __func_validate(gpt_response, prompt=""):
        try:
            return len(__func_clean_up(gpt_response, prompt="")) == 2
        except Exception:
            return False
...
def run_gpt_prompt_act_obj_event_triple(act_game_object, act_obj_desc, persona, verbose=False):
    ...
    def __func_clean_up(gpt_response, prompt=""):
        cr = gpt_response.strip()
        # Same fault as run_gpt_prompt_event_triple.
        cr = [i.strip() for i in cr.lstrip("(").split(")")[0].split(",")]
        if len(cr) > 2:
            cr = cr[-2:]
        return cr
...
def run_gpt_prompt_new_decomp_schedule(
    ...
    def __expected_minutes(prompt):
        """
        How many minutes the replacement schedule has to cover.

        The last block of the day, "from 22:00 PM to 00:00 AM", came
        out as -1320 minutes rather than 120. Add a day when the end is
        not after the start.
        """
        try:
            x = prompt.split("\n")[0].split("originally planned schedule from")[-1].strip()[:-1]
            a, b = [datetime.datetime.strptime(i.strip(), "%H:%M %p") for i in x.split(" to ")]
            if b <= a:
    ...
    def __func_clean_up(gpt_response, prompt=""):
        ...
        # Read only the lines that are schedule entries.
        ret = []
        for i in new_schedule:
            ...
            window = time_str.split(" ~ ")
            if len(window) < 2:
                continue
            try:
                start = datetime.datetime.strptime(window[-2].strip(), "%H:%M")
                end = datetime.datetime.strptime(window[-1].strip(), "%H:%M")
            except ValueError:
                continue
        ...
        # The validator requires the schedule to reach a set time,
        # otherwise a mechanically generated fallback is used. Now, a
        # schedule that is too short isn't regarded as wrong, and the
        # difference is absorbed into the last block.
        if ret:
            expected = __expected_minutes(prompt)
            if expected is not None:
                drift = expected - sum(d for _, d in ret)
                if drift and ret[-1][1] + drift >= 0:
                    ret[-1][1] += drift
        ...
        try:
            ...
            for act, dur in gpt_response:
                ...
                if str(type(dur)) != "<class 'int'>":
                    return False
            delta_min = __expected_minutes(prompt)
            if delta_min is not None and int(dur_sum) != int(delta_min):
                return False
...
def run_gpt_prompt_decide_to_talk(persona, target_persona, retrieved, test_input=None, verbose=False):
    ...
    #     except:
    #         return False
    def __read_answer(gpt_response):
        """
        Find the yes/no at the end of the reasoning.

        Upstream split on the literal `"Answer in yes or no:"`. The
        prompt template writes that line with quotes around the options
        (`Answer in "yes" or "no":`) so the split did not match, the
        whole reply came back as the "answer", and not the word yes or
        no.

        Better matching, otherwise falling back to the last standalone
        yes/no anywhere in the reply.
        """
        label = re.search(r'answer\s+in\s+"?yes"?\s+or\s+"?no"?\s*:\s*"?(yes|no)"?', gpt_response, re.IGNORECASE)
        if label:
            return label.group(1).lower()
        words = re.findall(r"\b(yes|no)\b", gpt_response, re.IGNORECASE)
        return words[-1].lower() if words else ""

    def __func_validate(gpt_response, prompt=""):
        return __read_answer(gpt_response) in ("yes", "no")

    def __func_clean_up(gpt_response, prompt=""):
        return __read_answer(gpt_response)

    def get_fail_safe():
        fs = "yes"
        return fs

    # max_tokens was 20, but 20 tokens is not enough to reach the
    # answer, and so the decision often fell to the fail-safe "yes".
    gpt_param = {
        "engine": "text-davinci-003",
        "max_tokens": 150,
        "temperature": 0,
...
def run_gpt_prompt_decide_to_react(persona, target_persona, retrieved, test_input=None, verbose=False):
    ...
    def __read_option(gpt_response):
        """
        Find which option the model chose.

        Upstream required an answer to be 1, 2, or 3. But, the prompt
        asks for reasoning before answering while `max_tokens` was 20,
        so calls were cut off mid-sentence and never reached an answer,
        and a chat model does not reliably reproduce the exact label.
        The fail-safe "3" kept getting used.
        """
        labelled = re.search(r"answer\s*:?\s*option\s*([123])", gpt_response, re.IGNORECASE)
        if labelled:
            return labelled.group(1)
        loose = re.findall(r"option\s*([123])", gpt_response, re.IGNORECASE)
        return loose[-1] if loose else ""

    def __func_validate(gpt_response, prompt=""):
        return __read_option(gpt_response) in ("1", "2", "3")

    def __func_clean_up(gpt_response, prompt=""):
        return __read_option(gpt_response)

    def get_fail_safe():
        fs = "3"
        return fs

    # max_tokens raised from 20 to 150.
    gpt_param = {
        "engine": "text-davinci-003",
        "max_tokens": 150,
        "temperature": 0,
...
def run_gpt_prompt_create_conversation(persona, target_persona, curr_loc, test_input=None, verbose=False):
    def create_prompt_input(init_persona, target_persona, curr_loc, test_input=None):
        ...
        if init_persona.a_mem.seq_chat:
            if (
                int((init_persona.scratch.curr_time - init_persona.a_mem.seq_chat[-1].created).total_seconds() / 60)
                > 480
            ):
                prev_convo_insert = ""
...
def run_gpt_prompt_convo_to_thoughts(
    persona, init_persona_name, target_persona_name, convo_str, fin_target, test_input=None, verbose=False
):
    def create_prompt_input(init_persona_name, target_persona_name, convo_str, fin_target, test_input=None):
        prompt_input = [init_persona_name, target_persona_name, convo_str, init_persona_name, fin_target]
        return prompt_input
    ...
    prompt_input = create_prompt_input(init_persona_name, target_persona_name, convo_str, fin_target)
    prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_focal_pt(persona, statements, n, test_input=None, verbose=False):
    ...
    # ChatGPT Plugin ===========================================================
    def __chat_func_clean_up(gpt_response, prompt=""):  ############
        # The local model is answering with a real JSON list instead of
        # a string. Accepts either now.
        if isinstance(gpt_response, str):
            return ast.literal_eval(gpt_response)
        return list(gpt_response)

    def __chat_func_validate(gpt_response, prompt=""):  ############
        # Now calls __chat_func_clean_up
        try:
            return len(__chat_func_clean_up(gpt_response, prompt)) > 0
        except Exception:
            return False
...
def run_gpt_prompt_insight_and_guidance(persona, statements, n, test_input=None, verbose=False):
    ...
    def __func_clean_up(gpt_response, prompt=""):
        gpt_response = "1. " + gpt_response.strip()
        # Every line had to contain "(because of ", so one malformed
        # line discarded the whole reply. Skip the lines that are not
        # insights and keep the ones that are.
        ret = dict()
        for i in gpt_response.split("\n"):
            row = i.split(". ")[-1]
            ...
            evi_raw = [int(x) for x in re.findall(r"\d+", evi_raw)]
            if thought:
                ret[thought] = evi_raw
        if not ret:
            raise ValueError("no insights with evidence could be read from the reply")
        return ret
...
def run_gpt_prompt_agent_chat(
    maze, persona, target_persona, curr_context, init_summ_idea, target_summ_idea, test_input=None, verbose=False
):
...
def run_gpt_generate_iterative_chat_utt(
    maze, init_persona, target_persona, retrieved, curr_context, curr_chat, test_input=None, verbose=False
):
    def create_prompt_input(maze, init_persona, target_persona, retrieved, curr_context, curr_chat, test_input=None):
        ...
        init_iss = (
            f"Here is Here is a brief description of {init_persona.scratch.name}.\n{init_persona.scratch.get_str_iss()}"
        )
        prompt_input = [
            init_iss,
    ...
    def __chat_func_validate(gpt_response, prompt=""):
        ...
        # Two fields are expected: the utterance, and whether the
        # conversation ends. Cannot be fewer than two.
