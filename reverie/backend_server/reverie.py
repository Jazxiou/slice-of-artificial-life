...
# Set to 0 to disable.
AUTOSAVE_EVERY = globals().get("autosave_every", 8640)


def _step_body_seconds():
    """Better wall-clock computation."""
    return llm_trace.recorder.step_seconds if llm_trace.is_recording() else 0.0
...
class ReverieServer:
    def __init__(self, fork_sim_code, sim_code):
        ...
        for needed in ("movement", "environment"):
            os.makedirs(f"{sim_folder}/{needed}", exist_ok=True)
    ...
    def save(self):
        ...
        reverie_meta["persona_config"] = persona_ext.config()
    ...
    def start_server(self, int_counter):
        ...
        #
        # Interupting a run still saves it.
        loop_started = time.time()
        try:
            self._step_loop(int_counter, sim_folder, loop_started)
        except KeyboardInterrupt:
            print("\n[interrupted] saving before stopping ...")
            self.save()
            print(
                f"[interrupted] saved at step {self.step} ({self.curr_time}). "
                f"Type `fin` to exit, or `run N` to carry on."
            )
        return

    def _step_loop(self, int_counter, sim_folder, loop_started):
        ...
        while True:
            ...
            if check_if_file_exists(curr_env_file):
                ...
                if env_retrieved:
                    ...
                    llm_trace.step(self.step, time.time() - step_started)
                    ...
                    if AUTOSAVE_EVERY and self.step % AUTOSAVE_EVERY == 0:
                        self.save()
                        counts = {name: len(p.a_mem.id_to_node) for name, p in self.personas.items()}
                        llm_trace.world(self.step, self.curr_time.strftime("%Y-%m-%d %H:%M"), counts)
                        print(
                            f"[checkpoint] step {self.step}, {self.curr_time}; "
                            f"memories: {', '.join(f'{k.split()[0]} {v}' for k, v in counts.items())}"
                        )
        ...
        llm_trace.stage(llm_trace.WAITING_STAGE, (time.time() - loop_started) - _step_body_seconds())
    ...
    def open_server(self):
        ...
        print(f"[condition] {persona_ext.describe()}\n---")
        ...
        while True:
            ...
            try:
                if sim_command.lower() in ["f", "fin", "finish", "save and finish"]:
                    ...
                    # Confirm first.
                    answer = input(
                        f"`exit` DELETES {sim_folder} and everything saved in it. "
                        f"Type the simulation name to confirm, or anything else to cancel: "
                    )
                    if answer.strip() != self.sim_code:
                        print("Cancelled. Use `fin` to save and exit.")
                        continue
                    shutil.rmtree(sim_folder)
                    break
...
if __name__ == "__main__":
    ...
    # Confirm the langauge model is actually reachable before doing
    # anything else.
