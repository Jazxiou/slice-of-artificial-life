...
func _avoid_sam():
	...
	for angle in range(0, 360, 30):
		...
		for collision in result:
			...
			if body != self and (body.name == "Sam" or body.name == "sam"):
				sam_found = body
				break
	...
	if sam_found:
		var direction_away = (global_position - sam_found.global_position).normalized()
		movement_direction = (movement_direction + direction_away).normalized()
